...
export default function CpuLab() {
  ...
  const loadWithConfiguration=useCallback((text:string,nextConfig:ProjectDocument["configuration"],lines:number[]=[]):boolean=>{const c=coreRef.current;if(!c)return false;const result=JSON.parse(c.assemble(text)) as AssemblyResult;setAssembly(result);setSource(text);setRunning(false);if(!result.ok){setSourceDirty(true);return false;}c.loadProgram(text);c.resetWithJson(JSON.stringify(nextConfig));setForwarding(nextConfig.forwarding);setPredictor(nextConfig.predictor);setCacheEnabled(nextConfig.cacheEnabled);setLoadedSource(text);setSourceDirty(false);setSelectedSlot(null);setSelectedEvent(null);setReference(null);const validLines=new Set<number>();for(const line of lines){const index=result.sourceLines.findIndex(value=>value===line);if(index>=0){validLines.add(line);c.setBreakpoint(index*4,true);}}setBreakpointLines(validLines);refresh();return true;},[refresh]);
  const assembleSource=useCallback((text:string)=>loadWithConfiguration(text,currentConfiguration),[currentConfiguration,loadWithConfiguration]);
  ...
  const act=useCallback((fn:(c:CoreSimulator)=>void)=>{const c=coreRef.current;if(!c)return;fn(c);refresh();},[refresh]);
  const projectDocument=useCallback(()=>createProject(projectName,source,currentConfiguration,[...breakpointLines].sort((a,b)=>a-b)),[projectName,source,currentConfiguration,breakpointLines]);
