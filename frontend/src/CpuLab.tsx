...
import type { AssemblyResult, CoreEvent, CoreSimulator, CpuState, PipelineSlot, StageName, TimelineFrame } from "./types";
...
function Inspector({ tab, state, simulator, refresh, memoryAddress, setMemoryAddress }: { tab: Tab; state: CpuState; simulator: CoreSimulator; refresh: () => void; memoryAddress: number; setMemoryAddress: (a:number)=>void }) {
  const [editValue,setEditValue]=useState("0");
  const memory = (()=>{ try{return JSON.parse(simulator.readMemory(memoryAddress,64)) as number[];}catch{return [];} })();
  if(tab==="Registers") return <div className="register-grid">{state.registers.map((v,i)=><button key={i} className={i===0?"zero":""} title={`Signed: ${signed(v)} · Binary: ${(v>>>0).toString(2).padStart(32,"0")}`} onDoubleClick={()=>{const x=window.prompt(`Set r${i}`,String(signed(v)));if(x!==null){simulator.setRegister(i,Number(x)>>>0);refresh();}}}><span>r{i}</span><b>{hex(v)}</b><small>{signed(v)}</small></button>)}</div>;
  if(tab==="Memory") return <div className="memory-panel"><div className="memory-toolbar"><label>Address <input value={hex(memoryAddress)} onChange={e=>setMemoryAddress(Number(e.target.value)||0)} /></label><label>Word value <input value={editValue} onChange={e=>setEditValue(e.target.value)} /></label><button onClick={()=>{const v=Number(editValue)>>>0;simulator.writeMemory(memoryAddress,[v&255,(v>>>8)&255,(v>>>16)&255,(v>>>24)&255].join(","));refresh();}}>Write word</button></div><div className="hex-dump">{memory.map((v,i)=><span key={i} className={i<4?"recent":""}><small>{(i%16===0)?hex(memoryAddress+i,4):""}</small>{v.toString(16).padStart(2,"0")}</span>)}</div></div>;
...
export default function CpuLab() {
  ...
  const [forwarding,setForwarding]=useState("full"); const [predictor,setPredictor]=useState("two-bit"); const [cacheEnabled,setCacheEnabled]=useState(false);
  const [activeTab,setActiveTab]=useState<Tab>("Registers"); const [selectedSlot,setSelectedSlot]=useState<PipelineSlot|null>(null); const [selectedEvent,setSelectedEvent]=useState<CoreEvent|null>(null);
  ...
  const refresh=useCallback(()=>{const c=coreRef.current;if(!c)return;setState(JSON.parse(c.getState()));setTimeline(JSON.parse(c.getTimeline()));},[]);
  const config=useCallback(()=>JSON.stringify({forwarding,predictor,cacheEnabled}),[forwarding,predictor,cacheEnabled]);
  const assembleSource=useCallback((text:string)=>{const c=coreRef.current;if(!c)return false;const result=JSON.parse(c.assemble(text)) as AssemblyResult;setAssembly(result);setSource(text);setRunning(false);if(result.ok){c.loadProgram(text);c.resetWithJson(config());setLoadedSource(text);setSourceDirty(false);setBreakpointLines(new Set());setSelectedSlot(null);setSelectedEvent(null);refresh();return true;}setSourceDirty(true);return false;},[config,refresh]);
  const assembleProgram=useCallback(()=>assembleSource(source),[assembleSource,source]);

  useEffect(()=>{let live=true;createSimulator().then(c=>{if(!live){c.delete();return;}coreRef.current=c;setSimulator(c);setAssembly(JSON.parse(c.assemble(initialSource)));c.loadProgram(initialSource);c.resetWithJson(JSON.stringify({forwarding:"full",predictor:"two-bit",cacheEnabled:false}));refresh();}).catch(e=>setError(e.message));return()=>{live=false;coreRef.current?.delete();coreRef.current=null;setSimulator(null);};},[refresh]);
  useEffect(()=>{if(!running||!coreRef.current)return;const delay=Math.max(30,1050-speed*100);const id=window.setInterval(()=>{const c=coreRef.current;if(!c)return;c.stepCycle();refresh();const s=JSON.parse(c.getState()) as CpuState;if(s.halted||s.faulted||s.status==="breakpoint")setRunning(false);},delay);return()=>window.clearInterval(id);},[running,speed,refresh]);
  ...
  const act=(fn:(c:CoreSimulator)=>void)=>{const c=coreRef.current;if(!c)return;fn(c);refresh();};
  ...
  const explanation=selectedEvent?.message ?? (selectedSlot?.valid ? `${selectedSlot.stage} is processing dynamic instruction #${selectedSlot.id}: ${selectedSlot.assembly}. ${selectedSlot.stage==="ID"?`The register file supplies r${selectedSlot.rs1} and r${selectedSlot.rs2}.`:selectedSlot.stage==="EX"?`The ALU inputs are ${hex(selectedSlot.operandA)} and ${hex(selectedSlot.operandB)}, producing ${hex(selectedSlot.aluResult)}.`:selectedSlot.stage.startsWith("MEM")?`The memory path uses address ${hex(selectedSlot.memoryAddress)}.`:selectedSlot.stage==="WB"?`The writeback value is ${hex(selectedSlot.writeValue)}.`:"The next PC and prediction are selected here."}` : "Select a stage, timeline cell, or event to see the exact values and decision that produced it.");
  ...
  return <main className={`lab density-${density}`}>
    <header className="toolbar">
      ...
      <div className="toolbar-actions" aria-label="Simulator controls">
        <div className="control-group"><span>Program</span><button className="primary" onClick={assembleProgram} title="Assemble the editor text, load it into the CPU, and reset">Assemble</button><button onClick={()=>act(c=>{c.resetWithJson(config());setRunning(false);})} disabled={executionDisabled} title="Reset the currently loaded program">Reset</button></div>
        ...
        <details className="more-controls"><summary>More</summary><div><button disabled={executionDisabled||running} onClick={()=>act(c=>c.restorePreviousCycle())}><b>Undo cycle</b><small>Restore the previous CPU snapshot</small></button><button disabled={executionDisabled||running} onClick={()=>act(c=>c.runUntilBreakpoint(100000))}><b>Run to breakpoint</b><small>Stop before a marked source line</small></button><button disabled={executionDisabled||running} onClick={()=>act(c=>c.runUntilCompletion(100000))}><b>Run to completion</b><small>Run until HALT, fault, or limit</small></button><label>Execution speed<input type="range" min="1" max="10" value={speed} onChange={e=>setSpeed(Number(e.target.value))}/></label><label className="density-control">Interface density<select aria-label="Interface density" value={density} onChange={e=>setDensity(e.target.value as Density)}><option value="comfortable">Comfortable · larger</option><option value="compact">Compact · original</option></select></label></div></details>
      </div>
      <div className="run-status"><Metric label="Cycle" value={st.cycles}/><Metric label="Retired" value={st.retired}/><Metric label="CPI" value={st.cpi.toFixed(2)}/><div className={`status-pill status-${state.status}`}><i/>{state.status}</div><button className="guide-button" onClick={()=>setGuideOpen(true)} aria-haspopup="dialog">Guide</button></div>
    </header>
    ...
    <section className="workspace">
      <aside className="left-column">
        ...
        <details className="configuration"><summary><span><b>Processor configuration</b><small>{forwarding} forwarding · {predictor}</small></span><i>⌄</i></summary><div className="configuration-body">
          <label>Hazard handling<select value={forwarding} onChange={e=>setForwarding(e.target.value)}><option value="full">Full forwarding</option><option value="none">No forwarding</option><option value="manual">Manual / unsafe</option></select></label>
          <label>Branch predictor<select value={predictor} onChange={e=>setPredictor(e.target.value)}><option value="always-not-taken">Always not taken</option><option value="always-taken">Always taken</option><option value="one-bit">One-bit table</option><option value="two-bit">Two-bit saturating</option></select></label>
          <label className="switch"><span><b>Educational data cache</b><small>2-way, LRU, write-back</small></span><input type="checkbox" checked={cacheEnabled} onChange={e=>setCacheEnabled(e.target.checked)}/><i/></label>
          <p>Configuration changes take effect after applying and resetting.</p><button onClick={assembleProgram}>Apply & reset processor</button>
        </div></details>
      </aside>
      ...
      <section className="main-column">
        <div className="pipeline-panel"><div className="section-heading"><div><span className="eyebrow">CURRENT CYCLE · {st.cycles}</span><h2>Six-stage pipeline</h2></div><div className="legend"><span><i className="normal"/>active</span><span><i className="stall"/>stalled</span><span><i className="forward"/>forward</span><span><i className="flush"/>flushed</span></div></div>
          <div className="stage-flow">{STAGES.map((name,i)=><div className="stage-item" key={name}><StageCard slot={slots[i]??virtualIf} selected={selectedSlot?.stage===name} onSelect={()=>{setSelectedSlot(slots[i]);setSelectedEvent(null);}}/>{i<5&&<div className="stage-arrow">→<small>{name==="EX"?"result":name==="MEM2"?"data":"latch"}</small></div>}</div>)}</div>
        ...
        <Timeline frames={timeline} onSelect={s=>{setSelectedSlot(s);setSelectedEvent(null);}}/>
