...
export function createProject(name: string, source: string, configuration: ProjectDocument["configuration"], breakpointLines: number[]): ProjectDocument {
  return { format: "pipeline-lab-project", version: 2, name: name.trim() || "Untitled CPU project", source, configuration, breakpointLines };
}
...
export function parseProject(text: string): ProjectDocument {
  ...
  if (project.format !== "pipeline-lab-project" || !([1, 2] as unknown[]).includes(project.version)) throw new Error("Unsupported Pipeline Lab project format.");
  ...
  return { format: "pipeline-lab-project", version: 2, name: typeof project.name === "string" ? project.name.slice(0, 100) : "Imported CPU project", source: project.source, configuration, breakpointLines: lines };
}

export function createTrace(source: string, state: CpuState, timeline: TimelineFrame[], reference: ReferenceComparison | null) {
  return { format: "pipeline-lab-trace", version: 2, exportedAt: new Date().toISOString(), source, configuration: state.configuration, finalState: state, referenceComparison: reference, timeline };
}
